    ...
    return (session?.user?.name && path !== "/" ? (
            <div className="flex flex-row items-center justify-center space-x-8">
                <div className="w-32" onMouseEnter={() => {setTimeout(()=> {setLogout(true)}, 300)}} onMouseLeave={() => {setTimeout(()=> {setLogout(false)}, 100)}}>
                    <div className="bg-green-500 w-[40px] h-[40px] rounded-full flex justify-center items-center cursor-pointer emailIcon transition-all duration-700 ease-out hover:w-32" onClick={() => {signOut()}}>
